...
const transform: Transform<TSX> = async (root, options?: unknown) => {
  ...
  if (
    enabled.has("route-file-structure") &&
    shouldRunTransform(currentFilename, "route-file-structure")
  )
    tasks.push(nextToTanstackFileStructureTransform(root));
  if (
    enabled.has("route-groups") &&
    shouldRunTransform(currentFilename, "route-groups")
  )
    tasks.push(nextRouteGroupsTransform(root));
  if (
    enabled.has("api-routes") &&
    shouldRunTransform(currentFilename, "api-routes")
  )
